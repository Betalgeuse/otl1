BEGIN;
CREATE SCHEMA IF NOT EXISTS otl;

CREATE TABLE IF NOT EXISTS otl.profiles (
  team_id text NOT NULL,
  user_id text NOT NULL,
  start_date date,
  empty_color text NOT NULL DEFAULT '#EBEDF0' CHECK (empty_color ~ '^#[0-9A-Fa-f]{6}$'),
  written_color text NOT NULL DEFAULT '#9BE9A8' CHECK (written_color ~ '^#[0-9A-Fa-f]{6}$'),
  complete_color text NOT NULL DEFAULT '#216E39' CHECK (complete_color ~ '^#[0-9A-Fa-f]{6}$'),
  palette_revision numeric NOT NULL DEFAULT -1,
  PRIMARY KEY (team_id, user_id)
);

CREATE TABLE IF NOT EXISTS otl.goals (
  team_id text NOT NULL,
  user_id text NOT NULL,
  goal_date date NOT NULL,
  goal_text text NOT NULL CHECK (length(btrim(goal_text)) BETWEEN 1 AND 500),
  completed boolean NOT NULL DEFAULT false,
  revision numeric NOT NULL,
  PRIMARY KEY (team_id, user_id, goal_date),
  FOREIGN KEY (team_id, user_id) REFERENCES otl.profiles (team_id, user_id)
);
