import assert from 'node:assert/strict';import {buildBoard} from '../src/board.ts';import {isOptionalDay} from '../src/calendar.ts';import {boardLink,readBoardLink} from '../src/board-link.ts';
const palette={empty:'#EBEDF0',written:'#9BE9A8',complete:'#216E39'};
const ladder=[];
const weekdayDate=(ordinal)=>{let stamp=Date.parse('2026-01-05T00:00:00Z'),seen=0;while(true){const date=new Date(stamp).toISOString().slice(0,10);if(!isOptionalDay(date)){seen+=1;if(seen===ordinal)return date;}stamp+=86400000;}};
