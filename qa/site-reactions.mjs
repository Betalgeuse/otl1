...
const initialPreview = evaluate(`({hiddenRows:[...document.querySelectorAll('[data-daily-row]')].filter(row=>!row.classList.contains('is-visible')).length,board:new URL(document.querySelector('[data-daily-garden-cell]').src).pathname,button:document.querySelector('[data-daily-replay]').textContent.trim()})`);
assert.deepEqual(initialPreview, { hiddenRows: 7, board: "/assets/fictional-four-day-board-before-review.png", button: "하루 재생" });
...
const escaped = evaluate(`({visible:[...document.querySelectorAll('[data-daily-row]')].filter(row=>row.classList.contains('is-visible')).map(row=>row.dataset.dailyRow),board:new URL(document.querySelector('[data-daily-garden-cell]').src).pathname,announcement:document.querySelector('[data-daily-announcement]').textContent.trim()})`);
assert.deepEqual(escaped.visible, ["goal", "ack"]);
assert.equal(escaped.board, "/assets/fictional-four-day-board-before-review.png");
assert.match(escaped.announcement, /재생을 멈췄습니다/);
results.push({ escaped });
browser("set", "media", "light", "reduced-motion");
browser("reload");
const reduced = evaluate(`({rise:[...document.querySelectorAll('.rise')].every(e=>getComputedStyle(e).display==='none'),static:getComputedStyle(document.querySelector('.still-reactions')).display,staticSources:[...document.querySelectorAll('.still-reactions img')].map(e=>e.getAttribute('src')),visibleRows:[...document.querySelectorAll('[data-daily-row]')].filter(row=>row.classList.contains('is-visible')).length,board:new URL(document.querySelector('[data-daily-garden-cell]').src).pathname})`);
