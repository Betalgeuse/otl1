...
for (const width of [320, 375, 768, 1440]) {
  ...
  assert.equal(measured.stageHeight, width <= 760 ? 320 : 400);
  assert.equal(measured.share, true);
  assert.equal(measured.sections, 7);
  assert.equal(measured.risingAssets, 12);
  if (width === 375) {
...
assert.equal(reduced.static, "grid");
