...
mock.module("../src/community-store.ts", () => ({
  CommunityStore: class {
    ...
    async claimRecord(x) {
      const r = records.get(x.userId);
      if (r?.status !== "pending") return false;
      r.status = "claimed";
      return true;
    }
    async finishRecord(x, status) {
      records.get(x.userId).status = status;
    }
  },
}));
...
globalThis.fetch = async (url, options) => {
  if (url.endsWith("/emoji.list"))
    return Response.json({
      ok: true,
      emoji: { dance: "https://img/a", party: "https://img/b", cat: "https://img/c" },
    });
  if (url.endsWith("/reactions.add")) return Response.json({ ok: true });
  if (url.includes("/users.info?")) {
    assert.equal(options.method, "GET");
    const user = new URL(url).searchParams.get("user");
    return Response.json({
      ok: true,
      user: { id: user, is_bot: user === "UBOT", deleted: false, is_app_user: false },
    });
  }
  const body = JSON.parse(options.body);
  assert.ok(url.endsWith("/chat.postMessage"));
  posts.push(body);
  return Response.json({ ok: true, ts: "123.123" });
};
const event = {
  type: "event_callback",
  team_id: "TQA",
  event: {
    ...
    channel: "CTOWN",
    user: "UNEW",
    ts: "1789091995.608629",
  },
};
try {
  await handleCommunityEvent(event, env);
  assert.equal(posts.length, 1, "first townhall join must produce welcome");
  await handleCommunityEvent(event, env);
  await handleCommunityEvent(
    { ...event, event: { ...event.event, type: "member_joined_channel", subtype: undefined } },
    env,
  );
