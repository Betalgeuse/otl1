...
const env={COMMUNITY_ENABLED:'true',SLACK_TEAM_ID:'TQA',COMMUNITY_RELEASE_CHANNEL_ID:'CTOWN',COMMUNITY_PUBLIC_CHANNEL_ID:'CDAILY',SLACK_BOT_TOKEN:'fake',DATABASE_URL:'postgresql://test:test@test.neon.tech/db'};
...
try{
 ...
 assert.equal(posts.length,1);assert.match(posts[0].text,/<@UNEW>/);assert.equal(posts[0].channel,'CTOWN');
