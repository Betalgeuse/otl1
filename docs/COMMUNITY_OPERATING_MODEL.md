...
입력 위치가 DM이어도 **등록한 목표는 공개**다. 초안 대화와 고민은 자동 공개하지 않고, 등록 버튼 앞에 공개 채널을 명시한다. 이 경계는 공개를 요청한 사용자 요구와 HabitShare의 명확한 공유 범위 설정에서 도출한 설계다. [벤치마크](COMMUNITY_BENCHMARK.md)
