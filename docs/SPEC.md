...
목표를 저장하면 즉시 `goal_prompt / recorded` projection을 만듭니다. 이 잔디 칸은 작성 상태이며 완료 칸이 아닙니다. 따라서 목표만 남긴 회원도 해당 parent thread에서 잔디를 볼 수 있습니다. 목표 등록과 후기 등록의 잔디 게시 성공은 평일·주말·공휴일 구분 없이 P0 불변 조건입니다. 선택 참여일에는 목표를 등록하지 않은 사람의 칸을 만들지 않지만, 목표를 등록한 사람의 잔디는 반드시 게시합니다.
...
`#all-shareinfo`와 설정된 모든 `#chapter-*` 채널의 사람이 작성한 top-level 새 글만 한 번 처리합니다. 봇 글, 기존 글의 답글, 편집 이벤트는 무시합니다. 봇은 원문에 커스텀 이모지를 추가하고 같은 parent thread에 감사 답글을 먼저 남긴 뒤, Qwen으로 한 줄 요약과 관련 회원·자료·주제를 연결하는 한 줄 생각거리를 별도 답글로 남깁니다. Qwen이 실패해도 앞의 이모지와 감사 답글은 유지하며 요약·생각거리는 재시도합니다.
...
- OT1L은 확정된 피드백을 **As-Is / To-Be**로 제시합니다. 명확한 개선 요청은 재현 시각·빈도·단계를 꾸며내지 않고 `feedback_packet.v1`로 보존합니다. 접수 직후 원문에 `loading` 반응을 달고 GenQuant가 자동으로 수정·검증한 Draft PR을 만듭니다. OT1L이 As-Is·To-Be·수정 결과·변경 내용 링크와 **병합 승인** 버튼을 같은 스레드에 제시하며, Slack workspace admin 또는 owner가 승인하기 전에는 병합하지 않습니다. 병합 뒤 관리자와 원제보자에게 결과를 알리고 `white_check_mark`로 완료를 표시합니다.
...
| 피드백·Codex 인계 | `src/community-feedback-route.ts`, `src/community-feedback.ts`, `src/community-bug-start.ts`, `src/community-bug-dialogue.ts` |
