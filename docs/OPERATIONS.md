...
자기소개 전체 목록은 `all-self-introduction`의 Canvas가 단일 읽기 화면입니다. 자기소개 등록·수정이 확정되면 현재 공개 introduction revision 전체로 Canvas를 교체합니다. `자기소개 모두 보기`를 누를 때도 같은 동기화를 먼저 실행하므로 일시적 갱신 실패를 다음 조회에서 복구합니다. daily-scrum 상단에는 사용설명서·자기소개 모음·홈페이지 북마크와 네 가지 핵심 동작의 핀 메시지를 유지합니다.
