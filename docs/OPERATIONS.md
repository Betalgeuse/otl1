...
| 안내 | 평일 | 주말 |
| --- | --- | --- |
| 공통 목표 | 10시, 실행 시점의 daily-scrum 사람 회원을 한 글에서 멘션 | 토요일 10시에 선택 참여 글 한 번, 멘션 없음. 일요일은 보내지 않음 |
...
개인 안내는 신규 설정의 기본값이 켜짐입니다. 본인이 시각을 변경하거나 끌 수 있고, 재입장이나 기본값 변경으로 다시 켜지지 않습니다. 신규 설정은 다음 날부터 대상이 되며 주말을 건너뜁니다. 오후 후기 안내는 목표가 있는 회원만 대상으로 합니다. 완료 표시와 후기 제출은 별개입니다. 발송 행은 lease로 claim하며 최대 세 번 시도합니다. 재시도 전에는 같은 채널에서 정확히 같은 본문을 찾아 이미 수락된 글이면 그 시각을 영수증으로 채택하고, 아직 due인 회원만 다시 남겨 전송합니다.
...
관리자는 설정에 지정된 **원본 Slack 안내글**을 편집한 뒤 `v0.0.55`처럼 증가한 버전으로 명시적으로 발행합니다. Slack 원문만 편집하면 신규 회원에게 전달되는 내용은 바뀌지 않습니다. 발행 작업은 Worker가 쓰는 런타임 DB 자격증명이 아니라 별도 관리자 DB 자격증명으로 실행하며, 관리자 작성자·정확한 원본 및 편집 시각·본문의 버전 표기·전체 채널 알림 한 번·서로 다른 Slack 파일 ID 두 개와 원문 첨부 순서·예상 canonical hash를 모두 검증합니다. DB도 등록된 발행자와 canonical hash를 다시 확인한 뒤 정제된 본문과 이미지 순서를 불변 발행본으로 고정합니다. 신규 회원 입장 시에는 변경 가능한 Slack 원문을 다시 읽지 않고 DB의 최신 `published` 버전만 전달합니다. 같은 회원·같은 해시는 다시 게시하지 않고, 새 해시는 같은 회원에게도 한 번 전달할 수 있습니다. 같은 버전의 다른 내용과 오래되거나 설정과 다른 원문은 거부합니다.

현재 초대 문구는 `같이하면 좋을 지인이 떠오르면 두 이미지와 아래 문장을 보내고 자유롭게 초대해주세요.`와 `매일 제일 중요한 일 하나 정해서 같이 끝내는 모임이야. 같이 할래?`입니다. 원본과 최신 업데이트 공지에는 전체 채널 알림을 한 번만 둘 수 있으며, 신규 회원별 자동 복사본에서는 `@channel` 일반 텍스트로 바꿔 다시 알리지 않습니다. Slack이 게시 요청을 수락했지만 응답을 잃은 경우 자동 재게시하지 않고 운영 대조 대상으로 남깁니다.
