...
**사용자 결정: 다양한 자연어를 이해하기 위해 LLM을 도입하며 비용 효율을 우선한다.** 문장·오늘 목표·스레드 문맥을 해석하고, 권한·저장·알림은 코드가 처리한다. 애매하면 ‘오늘 원씽을 완료하셨나요?’와 선택 버튼으로 확인한다. 모델 후보·요금·평가 기준은 [LLM 비용안](../research/LLM_COST_PLAN.md)을 따른다. 현재 연결·배포 완료를 뜻하지 않는다.
