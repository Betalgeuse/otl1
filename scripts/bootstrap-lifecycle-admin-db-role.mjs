...
for (const name of ["DATABASE_URL", "LIFECYCLE_ADMIN_SECRET_SINK", "LIFECYCLE_ADMIN_SECRET_SINK_ARGS"])
  if (!process.env[name]) throw new Error(`Missing ${name}`);

let sinkArgs;
try {
  sinkArgs = JSON.parse(process.env.LIFECYCLE_ADMIN_SECRET_SINK_ARGS);
  if (!Array.isArray(sinkArgs) || !sinkArgs.every((item) => typeof item === "string" && item.length > 0))
    throw new TypeError("invalid sink arguments");
} catch (error) {
  if (error instanceof SyntaxError || error instanceof TypeError) throw new Error("Invalid lifecycle admin sink arguments");
  throw error;
}
...
const sql = `BEGIN;
ALTER ROLE otl_lifecycle_admin_login PASSWORD '${escaped}';
COMMIT;`;
const dbEnv = {
  ...process.env,
...
if (applied.status !== 0) throw new Error("Lifecycle admin database role bootstrap failed; details withheld");

const adminUrl = new URL(ownerUrl);
adminUrl.username = "otl_lifecycle_admin_login";
adminUrl.password = password;
const delivered = spawnSync(process.env.LIFECYCLE_ADMIN_SECRET_SINK, sinkArgs, {
  input: `${adminUrl.toString()}\n`, encoding: "utf8", stdio: ["pipe", "pipe", "pipe"],
});
if (delivered.status !== 0) throw new Error("Lifecycle admin secret delivery failed; details withheld");
